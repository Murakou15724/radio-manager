...
`ApplicationController#require_login` により **未ログイン状態で `/`（ルート）にアクセスすると `/login` へ 302 リダイレクトされる**。Render のヘルスチェックがルートパス（`/`）に対して「200 が返ること」を期待する設定になっていると、302 を異常とみなしてデプロイが unhealthy 判定になる可能性がある。

このリポジトリには Rails 標準のヘルスチェック用ルート `/up`（`Rails::HealthController`）を追加してある。これは `ApplicationController` を経由しないため認証不要・DB アクセスなしで常に 200 を返す、最も軽量なエンドポイントである。Render の **Health Check Path** はこちらを設定すること。

- `/up`

死活監視サービス（cron-job.org など）から定期アクセスさせる場合も、`/`（DB更新の副作用がある）や `/login`（画面全体を描画する）ではなく、この `/up` を使うのが最も軽量である。
